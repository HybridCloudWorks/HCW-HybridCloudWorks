...
| Name | Status | Consumer |
| --- | --- | --- |
| `COPILOT_REVIEW_CLIENT_ID`, `COPILOT_REVIEW_TENANT_ID`, `COPILOT_REVIEW_SUBSCRIPTION_ID`, `COPILOT_REVIEW_APP_ID` | **NOT SET** — owner step 2b, [issue #381](https://github.com/HybridCloudWorks/HCW-HybridCloudWorks/issues/381) | To be created as copies of the four identifiers the repository variables already hold, because Copilot's own runner resolves no `vars.*` (seen on the first review session, PR #378, 2026-09-06) and reads only this store. `copilot-setup-steps.yml` prefers these and falls back to the variables, so a manual dispatch signs in today and a Copilot session will once they exist. Holding identifiers in a secret store is the documented exception in [Variables and secrets](variables-and-secrets.md#subscription-tenant-and-client-ids) |
