...
| Private endpoint (Cosmos) | ~~`pep-web-cosmos-prod-cus-01`~~ | **RESERVED, NOT BUILT** — no private endpoint exists; the posture is service endpoints plus default-`Deny`. [ADR 0008](../decisions/0008-selective-private-link.md) is unimplemented. The `web` token here is also off-convention |
