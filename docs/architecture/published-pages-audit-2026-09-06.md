...
| `/azure/blog/microsoft-foundry-end-to-end-observability-and-roi-for-production-ai-agents` | one broken image | Fixed in code, and it was not in the body. The broken `<img>` was the **hero**, resolved from `contentImageUrl`, which the curated feed had filled with `…/2026/06/RUBRIC-EVALUATOR.mp4` — a real 5.3 MB `video/mp4`. The body was never involved: the field that renders is the RSS stub in `Content`, which carries no images, and the `.mp4` appears in the unrendered `content` field only as a markdown *link*, which is fine. So re-hosting (#374) was never going to reach it. Cover resolution now rejects a URL whose media type is video (`isVideoUrl` in `frontend/src/lib/blogUtils.js`) and takes the next candidate instead, so the hero is omitted, `og:image` and `twitter:image` stop advertising a video to crawlers, and `twitter:card` falls back to `summary`. The check is on the extension or declared media type, not on that one URL, because the same feed will do this again. |
