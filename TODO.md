...
| `ai_insights` container | **Decided 2026-09-05: drop it.** The panel that read it was retired (T-765, #339) and nothing ever wrote it, so it held only the documents the 2026-08-21 migration carried across, with no reader. Two applies, because the container `prevent_destroy` guard covers every instance of the `for_each`: **(1)** #340 removes the manifest entry, regenerates `infra/cosmos-containers.json` and lifts the guard — approve its run at https://app.terraform.io/app/hcw/workspaces/hcw-azure/runs only if the plan reads **1 to destroy** and that one is `azurerm_cosmosdb_sql_container.hcw["ai_insights"]`. The permanent `azapi` diff (three resources replaced, Function App restart) is expected, as on every apply; any destroy or replace beyond that baseline means discard, not approve. **(2)** the PR that follows restores the guard, and rides with the Static Web App tier change (the row above). Irreversible after (1): no backup restore returns a dropped container's documents | Until (1) is applied the container is unchanged; until (2) is applied the guard is down, so no other container change may be merged in between |
