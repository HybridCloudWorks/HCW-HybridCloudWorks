...
| Static Web App tier | Re-justify the Standard tier (about USD 9/month) — carried here when T-721 closed on 2026-09-02. The in-file rationale (`infra/frontend.tf`) cites custom domain plus SSL, SPA routing and 100 GB bandwidth, all of which the Free tier also provides; the genuinely Standard-only features are the SLA and pull-request staging environments. Downgrade if neither is load-bearing | The tier stays Standard until decided; the change is one Terraform value and an approved run |
