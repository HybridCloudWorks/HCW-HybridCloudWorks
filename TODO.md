...
| **The Cosmos recovery objectives are untested by a restore.** RTO 8 h / RPO 24 h were approved and the nightly out-of-account export is proven (first run 2026-09-09 03:00 UTC: 53 containers, 1,951 documents, 33.7 MiB gzip, event and alert wired), but no restore has been performed or timed. Closed #231 and #455 | Owner, 2026-09-09 | The owner decided the export run and the three reads confirming it were the tests needed, and that no drill is wanted. The objectives therefore remain targets rather than measurements; a restore that misses 8 h would be discovered during an incident, not before it. Compensating: the copy is written nightly and its absence pages within two days; `scripts/restore-cosmos-export.mjs --dry-run` counts what a restore would touch with no scratch account; `docs/runbooks/cosmos-restore.md` §6 stays paste-ready. Revisit if the platform starts generating documents at scale (the same trigger the 2026-08-30 objective decision named) |
