...
describe("Publer reconcile", () => {
  ...
  it("skips when not configured; ignores Key Vault literals", async () => {
    ...
    expect(
      await createPublerReconcile({ store: memStore(), client, now }).run(),
    ).toMatchObject({
      skipped: true,
...
describe("blog listings (Firecrawl)", () => {
  it("calls the v1 scrape endpoint with the schema and resolves relative URLs", async () => {
    ...
    expect(resolveArticleUrl("x/y", "https://h.test/l/")).toBe(
      "https://h.test/x/y",
    );
    ...
    await expect(
      scrapeListingPage({ apiKey: "fc", fetch: failing }, "https://s"),
    ).rejects.toThrow("Firecrawl scrape failed for https://s: quota");
  });
...
describe("podcasts", () => {
  ...
  const feeds = [
    { provider: "azure", url: "https://feeds.example.test/hcw.xml" },
  ];
  it("builds episodes from enclosure or media:content, keeps createdAt across upserts, isolates item errors", async () => {
    expect(normalizePodcastId("https://podbean.com/e/Ep-1!", "t")).toBe(
      "podbean-com-e-ep-1",
    );
    const item = {
      guid: "g1",
    ...
    const store = memStore({
      podcasts: [
        { id: "g1", createdAt: "2026-01-01T00:00:00.000Z", title: "old" },
      ],
    });
    const parser = {
    ...
    expect(r.azure.errors).toEqual([
      { position: 3, error: expect.any(String) },
    ]);
    ...
    expect(brokenLog.error.mock.calls[0][0]).toBe(
      "[fetchPodcastFeeds] azure: feed failed: 504",
    );
  });
  ...
  it("fetchPodcastFeeds says so at Warning when the feed is reachable but empty", async () => {
    ...
    expect(log.warn).toHaveBeenCalledWith(
      "[fetchPodcastFeeds] azure: feed returned no items",
    );
    expect(log.error).not.toHaveBeenCalled();
  });
  ...
  it("treats HTTP 410 as the feed being gone: Warning, skipped, no Error (#348)", async () => {
    ...
    expect(isFeedGoneError(new Error("410 Gone but phrased differently"))).toBe(
      false,
    );
    const store = memStore({ podcasts: [] });
    const parser = {
  ...
  it("reads the feed list from admin_config/podcast_feeds, dropping rows it cannot use", async () => {
    ...
    expect(log.log.mock.calls[0][0]).toMatch(
      /^\[fetchPodcastFeeds\] \(admin_config\) /,
    );
  });
