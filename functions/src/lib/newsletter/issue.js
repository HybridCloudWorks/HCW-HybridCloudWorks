 ...
 *
 * Replaces `content/digest.js`, which drafted a markdown article from titles
 * and summaries with no links, into a container nothing ever read. An issue is
 ...
 * owner can add before approving. The email is rendered from that at approval,
 * so editing the note never means editing HTML.
 *
 * Building never sends. An issue is created `draft`; only an approval in the
 * admin schedules it (ADR 0029 §1b's rule, kept by the owner on 2026-09-13).
 *
