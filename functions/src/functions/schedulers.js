...
import {
  ...
  buildDedupFields,
} from "../lib/cms/content-dedup.js";
import {
  createPublerClient,
  createPublerReconcile,
} from "../lib/timers/publer-sync.js";
import { createBlogListingsScrape } from "../lib/timers/blog-listings.js";
import {
  createPodcastIngest,
  createPodcastParser,
} from "../lib/timers/podcasts.js";
...
function timer(name, flag, schedule, run) {
  app.timer(name, {
    ...
    handler: async (_timer, context) => {
      ...
      if (result !== undefined)
        context.log(`[${name}] ${JSON.stringify(result)}`);
    },
  });
}
...
timer(
  ...
  async (context) => {
    ...
    await createScheduledPublisher({ store, publish }).runScheduledPublish(
      context,
    );
  },
);
...
timer(
  "monitorPublishingPipeline",
  "MONITOR_PUBLISHING_PIPELINE",
  "0 0 */6 * * *",
  (context) => createPublishingWatchdog({ store, log: context }).run(),
);

timer(
  "generateReviewerDigest",
  "GENERATE_REVIEWER_DIGEST",
  "0 0 7 * * *",
  (context) => createReviewerDigest({ store, log: context }).run(),
);
...
timer(
  "cleanupUnusedCertImages",
  "CLEANUP_UNUSED_CERT_IMAGES",
  "0 0 5 * * *",
  (context) =>
...
timer(
  "reVerifyCertifications",
  "REVERIFY_CERTIFICATIONS",
  "0 0 0 * * 0",
  (context) => {
...
timer(
  "syncSocialCalendarScheduled",
  "SYNC_SOCIAL_CALENDAR",
  "0 */5 * * * *",
  (context) =>
