...
- **Cloud pricing comparison, Phase 3 backend: price history, a newsletter
  ...
  - **"Explain this number"**: `POST public/cloud-tools/explain`
    ...
    before it is stored.
- **Cloud pricing comparison, Phase 3 frontend: price changes, explain,
  second region, article embeds (#613).** Four additions to
  `/tools/comparison` and one to articles, against the two Phase 3 routes the
  backend PR adds: `GET public/cloud-tools/price-changes` and
  `POST public/cloud-tools/explain`. Until that PR is deployed the card and
  the button show the server's error rather than a number.
  - **Price changes** (`pages/tools/PriceChanges.jsx`), under the table: what
    moved in the region's list prices over the last 7 or 30 days, one row per
    move — "AWS · Virtual machine $0.192 → $0.201 /hour ↑ +4.7%", rises and
    falls coloured and signed — with "No changes in the last 7 days" and "No
    history yet" as their own states and an as-of line. Follows `?region=`.
  - **Explain this comparison** (`scenario/ExplainButton.jsx`): posts the
    computed comparison (provider, total, base, segments, unavailable — no
    lines, so it stays under the server's 8 KB cap) and shows the two
    paragraphs under "Generated by AI from the numbers above — check the
    sources before relying on it", with the model, the time and a note when
    the answer was cached. Only on a click, never on load; disabled while any
    provider is unavailable; 429 reads "try again in a while", 503 shows the
    server's sentence, anything else offers Retry. The answer is keyed to the
    numbers it explained and goes away when they change.
  - **Compare with** (`scenario/CompareRegion.jsx`): a second-region select
    on the results, state in `?compare=`, that fetches that region once and
    puts "US West: $782.10 (+10%)" under each provider's total and "Cheapest
    in US West: Google Cloud" under the list; loading and failure inline. An
    id the API did not list, or the region already shown, is ignored and
    dropped from the URL on the next change; a previous region's answer is
    never shown as the new one's. `lib/pricingScenarios/compare.js` holds the
    arithmetic; `share.js` carries `compare=` with the rest of the scenario.
  - **Article embeds** (`components/content/PricingScenarioEmbed.jsx`): a
    fenced block with language `pricing-scenario` whose body is a scenario
    query string renders the scenario card read-only inside a post, with an
    "Open in the comparison tool" link. Wired through the `code` override
    every markdown renderer shares (`components/shared/CodeBlock.jsx`), so
    blog posts, RichTextBody and the ContentForge preview all get it; the
    card is a `React.lazy` chunk a post without the fence never loads, and a
    post with it pre-renders and hydrates cleanly (test with `renderToString`
    → `hydrateRoot`). Documented in `docs/content/blog-template.md`.
