...
- **Landing Zone Builder, Phase 4: "Explain this component" and the
  ...
  (503) as sentences with no retry, the way the pricing button does. A
  `landing-zone` code fence whose body is the build's share query string
  (`lz=mg,policy,mgmt,hub,fw&corp=2`) renders a read-only card in an
  article: `components/content/LandingZoneEmbed.jsx` is the lazy shell and
  `LandingZoneCard.jsx` the chunk, decoding with `decodeLz`, drawing the
  diagram through the page's own SVG (moved into `landingZone/LzSvg.jsx`,
  with a non-interactive mode that leaves the boxes as plain groups), listing
  the selected components with a line each, and linking to the same build in
  the tool. `markdownCodeComponents` in `components/shared/CodeBlock.jsx`
