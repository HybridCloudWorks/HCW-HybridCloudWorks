...
function EpisodeImage({ image, title, size = 'md' }) {
  ...
  return (
    <div
      className={`${sizeClass} rounded-lg bg-gradient-to-br from-red-600/30 to-blue-900/40 border border-red-500/20 flex items-center justify-center flex-shrink-0`}
    >
      <span className="material-symbols-outlined text-red-400/60 text-3xl">podcasts</span>
    </div>
  );
}
...
export default function PodcastPage() {
  ...
  return (
    <>
      <Helmet>
        <title>Google Cloud Podcast | HCW</title>
        <meta
          name="description"
          content="Expert audio episodes on Google Cloud architecture, data platforms, and enterprise solutions."
        />
        <meta property="og:title" content="Google Cloud Podcast Series" />
        <meta
          property="og:description"
          content="Deep-dive podcast discussions on Google Cloud architecture and design."
        />
      </Helmet>
      ...
      <main className="flex-grow pt-28 pb-20 px-4 md:px-8 max-w-[1440px] mx-auto w-full">
        ...
        <section className="mb-10 relative">
          <div className="absolute -top-10 -left-10 w-96 h-96 bg-red-500/5 blur-3xl rounded-full pointer-events-none" />
          <h1 className="text-3xl sm:text-5xl md:text-6xl font-bold text-white mb-4 relative z-10">
            <span className="bg-clip-text text-transparent bg-gradient-to-r from-gcp-primary via-slate-900 to-gcp-primary dark:via-white">
              Google Cloud Podcast
            </span>
          </h1>
          <p className="text-base sm:text-lg text-foreground max-w-3xl relative z-10">
            Deep-dive podcast discussions on Google Cloud architecture, data platforms, and
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-8">
          ...
          <div className="space-y-8">
            {/* Featured Player */}
            {loading && (
              <div className="bg-card/40 border border-red-500/20 rounded-2xl p-8 text-foreground animate-pulse h-64" />
            )}

            {!loading && featured && (
              <article className="bg-card/40 backdrop-blur-md border border-red-500/30 rounded-2xl overflow-hidden">
                ...
                <div className="p-6 sm:p-8">
                  <div className="flex items-center gap-2 mb-4">
                    <span className="px-3 py-1 bg-red-500/20 border border-red-500/30 text-red-400 text-xs font-bold rounded">
                      Now Playing
                    </span>
                  </div>
                  ...
                  >
                    <div
                      className="h-full bg-gradient-to-r from-red-500 to-blue-400 rounded-full transition-all duration-200"
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      className="w-12 h-12 bg-gradient-to-br from-red-500 to-blue-600 hover:from-red-400 hover:to-blue-700 rounded-full flex items-center justify-center transition-all shadow-lg shadow-red-500/25 disabled:opacity-40"
                    >
            ...
            {!loading && !featured && (
              <div className="bg-card/40 border border-red-500/20 rounded-2xl p-8 text-foreground text-sm">
                      ...
                      className={`w-full text-left flex items-center gap-4 p-3 rounded-xl border transition-all duration-200
                        ${
                          isSelected
                            ...
                            : 'bg-card/30 border-card/50 hover:bg-card/50 hover:border-red-500/20'
                        }`}
                    >
                      <EpisodeImage image={ep.image} title={ep.title} size="sm" />
                      <div className="flex-1 min-w-0">
                        <p
                          className={`text-sm font-semibold truncate ${isSelected ? 'text-red-400' : 'text-white'}`}
                        >
                          {ep.title}
                        </p>
                      ...
                      {isSelected && isPlaying && (
                        <span className="material-symbols-outlined text-red-400 text-[18px] flex-shrink-0">
                          graphic_eq
                        </span>
                      )}
                    </button>
          ...
          <aside className="h-fit sticky top-28">
            <div className="bg-gradient-to-br from-red-500/20 to-blue-900/20 backdrop-blur-md border border-red-500/30 rounded-2xl p-6">
              <h3 className="text-lg font-bold text-white mb-2 flex items-center gap-2">
                <span className="material-symbols-outlined text-red-400 text-[20px]">podcast</span>
                Subscribe Now
              </h3>
                ...
                {platforms.map((platform) => {
                  ...
                  return (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-full py-2.5 px-3 bg-card/50 hover:bg-red-500/20 hover:border-red-500/40 border border-card/60 text-foreground rounded-lg transition-all text-sm font-semibold flex items-center gap-3"
                    >
                      <img
