...
function EpisodeImage({ image, title, size = 'md' }) {
  ...
  return (
    <div
      className={`${sizeClass} rounded-lg bg-gradient-to-br from-blue-600/30 to-blue-900/40 border border-blue-500/20 flex items-center justify-center flex-shrink-0`}
    >
      <span className="material-symbols-outlined text-blue-400/60 text-3xl">podcasts</span>
    </div>
  );
}
...
export default function PodcastPage() {
  ...
  const { episodes = [], loading } = usePodcastData('azure');
  ...
  return (
    <>
      <Helmet>
        <title>Azure Podcast | HCW</title>
        <meta
          name="description"
          content="Expert audio episodes on Azure architecture, cloud design patterns, and enterprise solutions."
        />
        <meta property="og:title" content="Azure Podcast Series" />
        <meta
          property="og:description"
          content="Deep-dive podcast discussions on Azure cloud architecture and design."
        />
      </Helmet>
      ...
      <main className="flex-grow pt-28 pb-20 px-4 md:px-8 max-w-[1440px] mx-auto w-full">
        ...
        <section className="mb-10 relative">
          ...
          <h1 className="text-3xl sm:text-5xl md:text-6xl font-bold text-white mb-4 relative z-10">
            <span className="bg-clip-text text-transparent bg-gradient-to-r from-azure-primary via-slate-900 to-azure-primary dark:via-white">
              Azure Podcast
            </span>
          </h1>
          <p className="text-base sm:text-lg text-foreground max-w-3xl relative z-10">
            Deep-dive podcast discussions on Azure cloud architecture, design patterns, and
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-8">
          ...
          <div className="space-y-8">
            {/* Featured Player */}
            {loading && (
              <div className="bg-card/40 border border-primary/20 rounded-2xl p-8 text-foreground animate-pulse h-64" />
            )}

            {!loading && featured && (
              <article className="bg-card/40 backdrop-blur-md border border-primary/30 rounded-2xl overflow-hidden">
                {/* Audio element — captions not applicable for podcast audio-only content */}
                ...
                <div className="p-6 sm:p-8">
                  ...
                  >
                    <div
                      className="h-full bg-gradient-to-r from-primary to-blue-400 rounded-full transition-all duration-200"
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      className="w-12 h-12 bg-gradient-to-br from-primary to-blue-700 hover:from-blue-500 hover:to-blue-800 rounded-full flex items-center justify-center transition-all shadow-lg shadow-primary/25 disabled:opacity-40"
                    >
            ...
            {!loading && !featured && (
              <div className="bg-card/40 border border-primary/20 rounded-2xl p-8 text-foreground text-sm">
                      ...
                      className={`w-full text-left flex items-center gap-4 p-3 rounded-xl border transition-all duration-200
                        ${
                          isSelected
                            ...
                            : 'bg-card/30 border-card/50 hover:bg-card/50 hover:border-primary/20'
                        }`}
                    >
                      <EpisodeImage image={ep.image} title={ep.title} size="sm" />
                      <div className="flex-1 min-w-0">
                        <p
                          className={`text-sm font-semibold truncate ${isSelected ? 'text-primary' : 'text-white'}`}
                        >
                          {ep.title}
                        </p>
