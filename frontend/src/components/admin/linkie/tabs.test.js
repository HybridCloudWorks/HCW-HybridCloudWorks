...
describe('resolveTab', () => {
  ...
  it('sends an old address to where its content went', () => {
    ...
    expect(resolveTab('stats')).toBe('analytics');
  });

  it('falls back to Links for an unknown id, for nothing, and for undefined', () => {
...
describe('tabHref', () => {
  it('builds a deep link on the hub route', () => {
    expect(tabHref('links')).toBe('/admin/linkie?tab=links');
  });

  it('encodes a value rather than pasting it into the query string', () => {
    expect(tabHref('a b&c')).toBe('/admin/linkie?tab=a%20b%26c');
  });
});
